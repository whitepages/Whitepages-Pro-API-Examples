...
class Location
{
    ...
    public function data()
    {
        return array(
            ...
            'legal_entities_at' => $this->legal_entities_at
        );
    }

    public function legal_entities_at()
    {
        $legal_entities_array = array();
        if (!empty($this->entity['legal_entities_at'])) {
            while (list(, $legal_entity) = each($this->entity['legal_entities_at'])) {

                //$person = new Person($json_response['dictionary'][$legal_entity['id']['key']]);
                array_push($legal_entities_array, $legal_entity['id']['key']);
            }
        }

        return $legal_entities_array;
    }
